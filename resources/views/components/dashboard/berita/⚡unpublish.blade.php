...
<x-ui.modal name="unpublish-article">
  <div class="text-center">
    <div class="z-1 relative mb-7 flex items-center justify-center">
      <svg class="fill-warning/15" width="90" height="90" viewBox="0 0 90 90" fill="none"
      ...
      <span class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
        <svg class="fill-warning" width="38" height="38" viewBox="0 0 38 38" fill="none"
          xmlns="http://www.w3.org/2000/svg">
    ...
    <h4 class="sm:text-title-sm mb-2 text-2xl font-semibold text-gray-800">
      Konfirmasi Unpublish
    </h4>
    <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
      Yakin mau batalkan publish berita <span class="font-semibold">{{ $article?->title }}</span>?
    </p>
    <div class="mt-7 flex w-full items-center justify-center gap-3">
      <button
        class="btn btn-warning flex w-full justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium text-white sm:w-auto"
        type="button" wire:loading.attr="disabled" wire:loading.class="btn-disabled" wire:click="unpublish">
        <div role="status" wire:loading wire:target="unpublish">
          <span class="loading loading-spinner loading-sm"></span>
        </div>
        <span>Unpublish Berita</span>
      </button>
    </div>
  </div>
</x-ui.modal>
