...
@section('main')
  <div class="relative">
    ...
    <div class="bg-base-100 relative mx-auto w-full rounded-3xl border p-8 shadow-sm">
      ...
      @endif
      <form class="space-y-6" method="POST" action="{{ route('guru-staff.store') }}" enctype="multipart/form-data">
        @csrf
          ...
          <div class="space-y-6">
            ...
            <div>
              ...
              <input class="input input-bordered @error('name') is-invalid @enderror w-full" id="name" name="name"
                type="text" value="{{ old('name') }}" placeholder="Contoh: Siti Aminah, S.Pd" />
              @error('name')
                <span class="helper-text">{{ $message }}</span>
              @enderror
            </div>
            ...
            <div>
              ...
              <input class="input input-bordered @error('position') is-invalid @enderror w-full" name="position"
                type="text" value="{{ old('position') }}" placeholder="Guru Matematika / Staff TU" />
              @error('position')
                <span class="helper-text">{{ $message }}</span>
              @enderror
            </div>
